...
export default {
  ...
  methods: {
    ...
    async simulateQueue(totalClientes) {
      const self = this; // asignar "this" a una variable
    let cola = new Queue();
    ...
    while (!cola.isEmpty()) {
        ...
        quedaronTransacciones = await cajero.atenderCliente(cliente); // pasar "self" como segundo parámetro
...
class Cajero {
    ...
    async atenderCliente(cliente) {
        ...
        while (this.transaccionesRealizadas <= this.maxTransacciones) {
            ...
            this.vueInstance.attendTrans(this.transaccionesRealizadas, cliente.id, this.maxTransacciones, cliente.hayTransacciones()); // Atendiendo transacción ${this.transaccionesRealizadas} del cliente ${cliente.id} con el cajero`
        }
  ...
  <div class="queue d-flex flex-column align-items-center justify-content-center w-100" >
    ...
    <div class="item-queue d-flex flex-column ">
      <div v-for="n in array" :key="n"  :class="{'btn-danger': idClient === n}" class="btn btn-primary my-2">
        Usuario: {{n}}
        <div v-for="i in maxTransacciones" :key="i" class="btn btn-light mx-2" :class="{'btn-dark': transaccionesRealizadas === i && idClient === n}" >
          {{ i }}
        </div>
        <div v-if="finish === false && maxTransacciones === transaccionesRealizadas && idClient === n">Finalizado</div>
  <div v-if="finish === true && maxTransacciones === transaccionesRealizadas && idClient === n">Aun tiene transacciones pendientes</div>
      </div>
    </div>
  </div>
</template>
